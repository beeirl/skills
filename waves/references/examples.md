...
```text
Dispatch 12 Grok workers with `paseo run -d` (batched to the concurrency bound) and `paseo wait` for all of them before synthesis.
...
- One `paseo run --new-workspace worktree` per attempt.
