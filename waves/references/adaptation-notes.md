...
## Codex-to-Paseo swaps (bee-mode, 2026-08-28)

This copy was re-targeted from Codex to bee-mode: Paseo 0.6.1 is the harness,
Claude Fable 5 (Paseo `claude` provider) is the manager, Grok 4.6 (Paseo
`grok` provider, `grok agent stdio`) is the only worker model.

| Codex idea | bee-mode replacement |
| --- | --- |
| `spawn_agent` / `wait_agent` / `send_message` / `followup_task` | `paseo run -d --json` / `paseo wait` / `paseo send` |
| `explorer`, `worker`, `default`, custom TOML roles | Role instructions inlined in the prompt; read-only vs worktree worker chosen by flags |
| `spawn_agents_on_csv` | One `paseo run` per row, `--output-schema` for JSON |
| `agents.max_threads`, `max_depth` | Concurrency bound of 4 Paseo agents per wave; `GROK_SUBAGENTS=0` for read-only workers |
| `update_plan` | The bee-mode task ledger plus Paseo agent titles |
| GPT-5.6 Sol/Terra/Luna tiers | Grok 4.6 at low/medium (scouting) or high (implementation); no cheaper tier |
| Codex app worktrees, `codex exec` | `paseo run --new-workspace worktree` |
| Codex `/review`, Auto-review | `bee-grok-review`, `interrogate` |
| Coordinator Thread Mode (Desktop) | Paseo agents are durable by default: `paseo ls`, `paseo attach`, `paseo schedule` |

The Cursor-to-Codex notes below are kept as history.
