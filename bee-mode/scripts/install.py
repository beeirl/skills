#!/usr/bin/env python3
"""Install or verify the bee-mode skill, its Grok wrappers, and Grok approve mode."""
...
SKILL_NAME = "bee-mode"
WRAPPER_NAMES = ("bee-grok", "bee-grok-review")
...
def tree_digest(root: Path) -> str:
    if not (root / "SKILL.md").is_file():
        raise HarnessError(f"not a bee-mode skill directory: {root}")
...
def replace_skill(source: Path, target: Path) -> None:
    ...
    staging_parent = Path(tempfile.mkdtemp(prefix=f".{SKILL_NAME}-stage-", dir=target.parent))
...
def main() -> int:
    ...
    for configured_root in configured_roots:
        ...
        resolved_target = (skills_root / SKILL_NAME).resolve()
    ...
    try:
        ...
        if arguments.command == "install":
            for skills_root in skills_roots:
                replace_skill(source, skills_root / SKILL_NAME)
