#!/usr/bin/env python3
"""Verify identical bee-mode/Grok installs locally and over SSH."""
...
REMOTE_VERIFY = "~/.agents/skills/bee-mode/scripts/install.py verify --json"
...
def parser() -> argparse.ArgumentParser:
    ...
    argument_parser.add_argument(
        ...
        nargs="*",
        default=["local"],
        help="Use 'local' or an SSH destination such as user@host.",
    )
    argument_parser.add_argument(
        "--source",
        type=Path,
...
def local_report(source: Path) -> dict[str, object]:
    verifier = source / "scripts" / "install.py"
