...
```bash
bin/agent-routes scan     # or the setup-bee-mode skill
```
...
T3 Code is a GUI over the same CLIs; the harness the scan detects is still Claude Code, Codex, or
Grok, so everything above applies unchanged. What T3 adds:

- The coordinator is the current thread. A worker is a separate thread the user starts with
  **New worktree** on; one worktree is one writer lease. The coordinator cannot start threads, so
  it writes the task packet and asks the user to open the worker thread and paste the receipt back.
  Claude's in-harness sub-agents are fine for same-family work; they never satisfy a
  different-family review gate.
- Permission mode is chosen per thread in the composer and overrides `~/.grok/config.toml`. Use
  Full access only inside a disposable worktree. The Grok approve-mode invariant in
  [`references/fallback-clis.md`](references/fallback-clis.md) applies to terminal `bee-grok`
  launches, not to T3 threads.
- For a different-family review, run `bee-grok-review` from the shell on the fixed artifact, or
  open a Grok thread on the worktree. Leave commit, push, and PR to T3's source-control action
  unless the user has authorized them in the thread.
