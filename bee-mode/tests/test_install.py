...
'''
    ...
    def test_install_is_idempotent_and_verifiable(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            first_digest = SYNC.tree_digest(skills_root / "bee-mode")
            ...
            self.assertEqual(SYNC.tree_digest(skills_root / "bee-mode"), first_digest)
            self.assertTrue((bin_directory / "bee-grok").stat().st_mode & 0o100)
            self.assertTrue((bin_directory / "bee-grok-review").stat().st_mode & 0o100)
    ...
    def test_default_install_converges_agents_and_claude_roots(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            agents_skill = home / ".agents" / "skills" / "bee-mode"
            claude_skill = home / ".claude" / "skills" / "bee-mode"
    ...
    def test_wrapper_rejects_permission_override(self) -> None:
        wrapper = Path(__file__).resolve().parents[1] / "scripts" / "bee-grok"
        result = subprocess.run(
    ...
    def test_review_wrapper_enforces_bounded_streaming_profile(self) -> None:
        wrapper = Path(__file__).resolve().parents[1] / "scripts" / "bee-grok-review"
        with tempfile.TemporaryDirectory() as directory:
            bin_directory = Path(directory)
            fake_grok = bin_directory / "grok"
            fake_grok.write_text("#!/bin/sh\nprintf '%s\\n' \"$@\"\n", encoding="utf-8")
