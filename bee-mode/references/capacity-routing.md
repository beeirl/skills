...
Start the interactive CLI through `bee-grok`, then open `/usage`. Record the weekly percentage,
local reset time, prepaid balance, and auto-top-up state that the panel exposes. The direct wrappers `bee-grok` and `bee-grok-review` and every Paseo-dispatched Grok agent share the
same provider pool unless the account view proves otherwise.
...
`paseo provider ls --json` and `paseo provider diagnostic grok --json` prove that a provider is
installed and can open a session. They do not prove request capacity; use the provider-native
evidence above. Grok agents dispatched through `paseo run` draw from the same Grok Build pool as
`bee-grok` and `bee-grok-review`.
