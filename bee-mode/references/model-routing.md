...
`bee-mode`. Repository instructions and explicit user authority override it.
...
- **Coordinator:** the single user-facing context accountable for intent, authority, acceptance
  criteria, the dependency graph, and final judgment. In bee-mode this is the Paseo `claude` agent
  running `claude-fable-5`.
- **Supervisor:** a read-only background lane that watches long-running checks, CI, bounded retries,
  and worker lifecycle. Paseo provides it: `paseo wait <id>`, `paseo logs <id>`, `paseo attach <id>`,
  `paseo permit ls`, `paseo stop <id>`.
...
- read-only or writer permissions, writer lease if any, and timeout;
- the Paseo agent ID and workspace ID when dispatched through Paseo; and
...
Every direct Grok CLI launch MUST use the installed `bee-grok` wrapper. The wrapper injects
`--always-approve` and rejects `--permission-mode` and a caller-supplied `--always-approve`. Agents
MUST NOT call the underlying `grok` binary directly. A Grok worker dispatched through Paseo runs
`grok agent stdio` under the daemon; the same configuration setting governs it, because Paseo exposes
no permission modes for the Grok provider. Pass `--env GROK_MEMORY=0` to every Paseo dispatch and
`--env GROK_SUBAGENTS=0` to read-only packets. The user-level Grok configuration MUST also
...
`bee-mode/scripts/install.py verify --json` and `paseo provider diagnostic grok`. A missing
wrapper, configuration drift, skill digest mismatch, or a Paseo diagnostic that is not `Ready` makes
the route `unavailable`; do not improvise another permission mode.
...
For a fixed-artifact, read-only Grok review, use `bee-grok-review`. It pins Grok 4.6 at low reasoning,
...
`bee-grok` directly when a Grok worker needs tools or deeper reasoning.
